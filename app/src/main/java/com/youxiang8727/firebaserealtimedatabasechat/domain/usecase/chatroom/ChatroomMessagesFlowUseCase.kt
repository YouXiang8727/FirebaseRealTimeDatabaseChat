...
class ChatroomMessagesFlowUseCase @Inject constructor(
    private val firebaseRealTimeDatabaseRepository: FirebaseRealTimeDatabaseRepository
) {
    operator fun invoke(chatroomId: String): Flow<Message> {
        return firebaseRealTimeDatabaseRepository.chatroomMessagesFlow(chatroomId = chatroomId)
            .flowOn(
                Dispatchers.IO
            )
    }
